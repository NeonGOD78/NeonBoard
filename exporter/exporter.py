...
gauges = {
    ...
    "neonboard_disk_root_percent_free": Gauge(
        "neonboard_disk_root_percent_free", "Percentage of free disk space on /"
    ),
    # Disk (downloads)
    ...
    "neonboard_disk_downloads_percent_free": Gauge(
        ...
        "Percentage of free disk space on downloads mount",
    ),
    # Temperatures
...
def metrics():
    ...
    try:
        ...
        try:
            ...
            downloads_usage = shutil.disk_usage("/")
        ...
        for name, pid, cpu, _ in sorted(processes, key=lambda x: x[2], reverse=True)[
            :5
        ]:
            gauges["neonboard_top_cpu_process_percent"].labels(
                name=name, pid=str(pid)
            ).set(cpu)
        for name, pid, _, mem in sorted(processes, key=lambda x: x[3], reverse=True)[
            :5
        ]:
